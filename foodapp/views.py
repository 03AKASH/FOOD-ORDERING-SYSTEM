...
def getfoodlist(request):
	var=request.GET['searchMe1']
	foods = Food.objects.raw('Select * from FP_Food where FoodName="%s" or HotelName="%s"'%(var,var))
	'''#return render(request,'foodlist.html',{'foodlist':foods})
	foods=Food.objects.raw('Select * from FP_Food where HotelName="Royals"' )

	return render(request,"searchedItem.html",{'listed':foods})'''

	return render(request,'searchedItem.html', {'foodlist': foods})
...
def showcart(request):
	cart=Cart.objects.raw('Select CartId,FoodName,FoodPrice,FoodQuant,FoodImage,FoodIngredients,FoodExpiry,HotelName from FP_Food as f inner join FP_Cart as c on f.FoodId=c.FoodId where c.CustEmail="%s"'%request.session['CustId'])
	transaction.commit()
...
def placeorder(request):
        if request.method=="POST":
                ...
                var=cursor.execute('select distinct(HotelName) from FP_Transaction')
                cursor.execute('update FP_Transaction set Amount="100" where HotelName="Royals"')
                print(var)
                ...
                for o in Order.objects.raw(sql1):
                        if o.CustEmail==request.session['CustId']:
                                od=str(o.OrderId)
                                return render(request,'ToAdmin.html',{'success':'Order placed sucessfully!!!'+str(o.OrderId),'keyAmount':total},)
